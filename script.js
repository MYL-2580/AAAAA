...
searchInput.addEventListener('input', function () {
    ...
    const matchingAttractions = attractions.filter(attraction => {
        return attraction.name.toLowerCase().includes(keyword);
    });
    ...
    matchingAttractions.forEach(attraction => {
        ...
        suggestionItem.textContent = attraction.name;
...
function performSearch(keyword) {
    ...
    displayRecommendations(recommendedAttractions);
}

// 显示推荐景点
function displayRecommendations(recommendedAttractions) {
    const recommendationsDiv = document.getElementById('recommendations');
    recommendationsDiv.innerHTML = '';
    if (recommendedAttractions.length === 0) {
        recommendationsDiv.innerHTML = '<p>没有找到符合条件的景点，请调整搜索条件。</p>';
    } else {
        recommendedAttractions.forEach(attraction => {
            const link = document.createElement('a');
            link.href = `detail.html?attraction=${encodeURIComponent(JSON.stringify(attraction))}`;
...
document.addEventListener('DOMContentLoaded', function () {
    ...
    searchButton.addEventListener('click', function () {
        ...
        localStorage.setItem('recommendations', JSON.stringify(matchedAttractions));
        window.location.href = 'recommendation.html';
    });
});
// 获取旅游推荐
function getTravelRecommendations() {
    ...
    form.addEventListener('submit', function (e) {
        ...
        const recommendedAttractions = attractions.filter(attraction => {
            return (
                attraction.location.includes(destination) &&
                attraction.type.includes(travelType) &&
                budget >= attraction.budget.min && // 使用新的预算字段
                budget <= attraction.budget.max &&
                attraction.suitable.includes(travelMode)
            );
        });
